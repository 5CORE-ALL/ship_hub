...
class ShopifySyncOrders extends Command
{
    ...
    protected $description = 'Sync orders from Shopify API';
    ...
    public function handle()
    {
        ...
        // $url = "https://{$apiKey}:{$password}@{$shopUrl}/admin/api/{$version}/orders.json";
        ...
        $url = "https://{$apiKey}:{$password}@{$shopUrl}/admin/api/{$version}/orders.json?status=any&created_at_min={$since}&limit=250";
...
//     Log::info('order_data: ' . json_encode($order, JSON_PRETTY_PRINT));
// }

        foreach ($orders as $o) {
            $orderNumber = $o['name'] ?? $o['id'];
            $tag_name = $o['tags'] ?? '';
            $marketplace = detectMarketplaceFromTags($tag_name);
            ...
            if (!empty($o['fulfillments'])) {
                // Look for the first non-cancelled fulfillment
                foreach ($o['fulfillments'] as $f) {
                    if ($f['status'] !== 'cancelled' && !empty($f['tracking_number']) && !empty($f['tracking_company'])) {
                        ...
                        break; // Stop after finding the first valid fulfillment
                    }
                }
            ...
            if (!empty($o['cancelled_at']) && $o['cancelled_at'] !== null) {
                $status = 'cancelled';
            ...
            $itemSkus = array_filter(array_map(function ($item) {
                return $item['sku'] ?? '';
            }, $o['line_items'] ?? []));

            $itemSkuForOrder = !empty($itemSkus) ? implode(', ', $itemSkus) : '';
 ...
 'recipient_name' => trim(
    ...
    ?? ($o['contact_email'] ?? ($o['customer']['email'] ?? '')),
                    // 'recipient_phone' => $o['shipping_address']['phone'] ?? '',
            ...
            if (!empty($o['fulfillments'])) {
                foreach ($o['fulfillments'] as $f) {
                    ...
                    if (empty($trackingNumber)) {
                        continue; // Skip fulfillments without tracking numbers
                    }
